...
class WhitelistClass {
    ...
    add(connection_id) {
        ...
        (0, common_1.emit)({ ch: 'network', msg: `whitelisted connections from ${connection_id}`, verbose: true });
    }
    remove(connection_id) {
        this.whitelisted_ids.delete(connection_id);
        (0, common_1.emit)({ ch: 'network', msg: `removed ${connection_id} from whitelist`, verbose: true });
    }
}
exports.default = async () => {
    const W = new WhitelistClass();
    const API = (0, common_1.registerMethods)({
        source: W,
        methods: [
            'firewall', 'add', 'remove'
        ],
