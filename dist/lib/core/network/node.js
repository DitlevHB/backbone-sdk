...
function startBootstrapper(port, cb) {
    ...
    node.ready().then(function () {
        console.log(`backbone://network bootstrap server running on port ${node.address().port} - ${node.firewalled ? 'Firewalled!' : 'Not firewalled, all good'}`);
        cb();
    });
}
;
(async () => {
    ...
    console.log(`backbone://network bootstrap server running on port 39999 - ${bootstrapper1.firewalled ? 'Firewalled!' : 'Not firewalled, all good'}`);
    async.forEach(ports, (port, done) => {
