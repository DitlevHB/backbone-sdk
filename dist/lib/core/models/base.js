...
async function default_1(data, opts, migrations) {
    ...
    function getMetaDetails(meta) {
        const public_key = (0, crypto_1.getPublicKeyFromSig)({
            message: (0, crypto_1.hex2buf)(meta.hash),
            signature: (0, crypto_1.hex2buf)(meta.signature),
        });
    ...
    class datamodel extends Model(data).assert((data) => {
        ...
        const hash = (0, crypto_1.createHash)((0, msgpackr_1.pack)(signable_data));
        if (data._initial) {
            return true;
        }
        else if (!(0, crypto_1.verify)(hash, signature, public_key)) {
            const og_id = (0, crypto_1.getIdFromPublicKey)((0, crypto_1.hex2buf)(public_key));
        ...
        constructor(data) {
            ...
            if (data?._meta) {
                ...
                (0, common_1.error)('_meta missing');
            }
            if (!opts?.disable_owneronly && data._meta?.unsigned === true)
                (0, common_1.error)("can't create object with unsigned data");
            super(data);
            this._meta = tmp_meta;
        ...
        async sign() {
            if (typeof this['_meta']?.unsigned === undefined)
                return (0, common_1.error)('data is already signed');
            await checkUser();
            if (!current_id)
                return (0, common_1.error)('tried to sign without being authenticated');
            let { public_key } = getMetaDetails(this['_meta']);
            let pid = (0, crypto_1.getIdFromPublicKey)((0, crypto_1.hex2buf)(public_key));
            if (!b4a_1.default.isBuffer(public_key))
                public_key = (0, crypto_1.hex2buf)(public_key);
            if (!b4a_1.default.isBuffer(current_id))
                current_id = (0, crypto_1.hex2buf)(current_id.match(/^0x/) ? current_id : '0x' + current_id);
            if (!b4a_1.default.equals(current_id, pid))
                return (0, common_1.error)("current id doesn't match the author");
    ...
    async function signObject(data) {
        ...
        let hash = (0, crypto_1.createHash)((0, msgpackr_1.pack)(signable_data));
        let signature = await app_meta.backbone().id.signObject({ hash });
        if (b4a_1.default.isBuffer(hash))
            hash = (0, crypto_1.buf2hex)(hash);
        if (b4a_1.default.isBuffer(signature))
            signature = (0, crypto_1.buf2hex)(signature);
        if (!signature) {
            return (0, common_1.error)(`signing object failed`);
        }
        else {
    ...
    async function checkUser() {
        if (!app_meta.backbone()?.id) {
            if (typeof app_meta.backbone()?.user === 'function') {
                ...
                return (0, common_1.error)('authentication required but no authentication method found');
            }
        }
        else {
            ...
            if (is_authenticated) {
                ...
                if (!current_id)
                    return (0, common_1.error)('error in getting user id');
            }
            else {
                await app_meta.backbone().user();
                await checkUser();
    ...
    return async (data) => {
        ...
        if (data?._meta) {
            if (!opts?.disable_owneronly && !data._meta.signature) {
                return (0, common_1.error)('signature and id required');
            }
        }
        else {
            if (!opts?.disable_owneronly) {
