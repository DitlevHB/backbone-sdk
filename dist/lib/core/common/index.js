...
function decodeCoreData(data) {
    return (0, msgpackr_1.unpack)(data);
}
...
function getHomedir() {
    if (bbconfig_1.user.home_dir === '~')
        return process.env.TEST ? `${(0, os_1.homedir)()}/.backbone-test` : `${(0, os_1.homedir)()}/.backbone`;
    else
...
function hash(params) {
    ...
    const hash = (0, crypto_1.createHash)(params.type);
    hash.update(txt);
...
async function verifySign(payload) {
    ...
    return await (0, crypto_2.verifySignature)({
