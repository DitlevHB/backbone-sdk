...
function getStorage(bb_config) {
    ...
    if (bb_config?.storage === 'ram') {
        (0, common_1.log)('RAM storage requested, using memory for storage');
        storage = random_access_memory_1.default;
    }
    else if (bb_config?.env === 'node' || platform_detect_1.default?.node) {
        (0, common_1.log)('Node runtime detected, using file system for storage');
        const prefix = bb_config?.storage_prefix ? `${bb_config?.storage_prefix}/` : '';
        if (prefix)
            (0, common_1.log)(`Using storage prefix: ${prefix}`);
        const pathname = bb_config.address.match(/.{1,2}/g)?.join('/');
        storage = process.env.TEST
            ? `${(0, os_1.homedir)()}/.backbone-test/${prefix}${pathname}`
            : `${(0, common_1.getHomedir)()}/${prefix}${pathname}`;
    }
    else {
        (0, common_1.log)('Browser runtime detected, using RAI for storage');
        storage = (0, random_access_idb_1.default)();
    }
