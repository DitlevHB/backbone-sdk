...
class CoreClass {
    constructor(config, protocol) {
        ...
        const { storage, storage_id } = (0, get_storage_1.default)(config);
    ...
    async init() {
        ...
        if (!this.address.match('backbone://'))
            this.address_hash = (0, crypto_1.createHash)(`backbone://${this.config.address}`);
        else
            this.address_hash = (0, crypto_1.createHash)(this.config.address);
        if (!this.config.id)
            this.mode = 'read';
        else {
            if (!this.config.key)
                return (0, common_1.error)(`Core key needs to be supplied if opened in write mode`);
        ...
        if (this.config?.encryption_key !== false && typeof this.config.encryption_key === 'string') {
            encryptionKey = b4a_1.default.from((0, crypto_1.createHash)(this.config.encryption_key), 'hex');
        ...
        if (this.config.key) {
            writer_conf['keyPair'] = (0, crypto_1.keyPair)((0, crypto_1.createHash)(this.config.key + 'writer'));
            index_conf['keyPair'] = (0, crypto_1.keyPair)((0, crypto_1.createHash)(this.config.key + 'index'));
            meta_conf['keyPair'] = (0, crypto_1.keyPair)((0, crypto_1.createHash)(this.config.key + 'meta'));
            meta_index_conf['keyPair'] = (0, crypto_1.keyPair)((0, crypto_1.createHash)(this.config.key + 'meta_index'));
        }
        else {
        ...
        this.writer_key = (0, common_1.buf2hex)(this.writer.key);
        this.index_key = (0, common_1.buf2hex)(this.index.key);
        this.meta_key = (0, common_1.buf2hex)(this.meta.key);
        async function getDataAPI(data) {
            return {
                put: async (params) => {
                    if (typeof params === 'string' || !params?.key || !params?.value)
                        return (0, common_1.error)('INVALID PARAMS');
                    const encoded_data = (0, common_1.encodeCoreData)(params.value);
                    ...
                    return (0, common_1.error)('PUT FAILED');
                },
                del: async (key) => {
                    return data.del(key);
                },
                get: async (key) => {
                    ...
                    if (!dat)
                        return null;
                    return (0, common_1.decodeCoreData)(dat.value);
                },
                query: async function (params) {
                    ...
                    return new Promise((resolve, reject) => {
                        const bundle = [];
                        stream.on('data', (data) => {
                            bundle.push((0, common_1.decodeCoreData)(data.value));
                        });
                        stream.on('end', () => {
                            resolve(bundle);
                        });
        ...
        this.metaviewer = new data_viewer_1.default({
            ...
            async apply(data, operations) {
                ...
                for (const { value } of operations) {
                    const o = (0, common_1.decodeCoreData)(value);
                ...
                for (const { value } of operations) {
                    ...
                    await self.protocol(op, DataAPI, this.mode === 'write'
                        ...
                        get: async (key) => {
                            ...
                            if (!data)
                                return null;
                            return (0, common_1.decodeCoreData)(data.value);
                        },
                        query: async function (params) {
                            if (!params?.limit)
                                params.limit = 100;
        ...
        (0, common_1.emit)({ ch: 'core', msg: `Initialized Core ${this.address}` });
        const kp = (0, crypto_1.keyPair)(this.address_hash);
        ...
        root.on('peer-add', (peer) => {
            addPeersExt.send({
                data: {
                    peers: this.dataviewer.inputs.map((core) => (0, common_1.buf2hex)(core.key)),
                    trusted_peers: this.dataviewer.outputs.map((core) => (0, common_1.buf2hex)(core.key)),
                },
                meta: {
                    peers: this.metaviewer.inputs.map((core) => (0, common_1.buf2hex)(core.key)),
                    trusted_peers: this.metaviewer.outputs.map((core) => (0, common_1.buf2hex)(core.key)),
                },
            }, peer);
            (0, common_1.emit)({
                ch: 'network',
        ...
        (0, common_1.emit)({
            ch: 'init',
            msg: `discovery keys:\nwriter: ${(0, common_1.buf2hex)(this.writer.discoveryKey)}\nindex: ${(0, common_1.buf2hex)(this.index.discoveryKey)}\nroot: ${(0, common_1.buf2hex)(root.discoveryKey)} \nmeta: ${(0, common_1.buf2hex)(this.meta.discoveryKey)}`,
            verbose: true,
        });
        (0, common_1.emit)({
            ch: 'init',
            msg: `public keys:\nwriter: ${(0, common_1.buf2hex)(this.writer.key)}\nindex: ${(0, common_1.buf2hex)(this.index.key)}\nroot: ${(0, common_1.buf2hex)(root.key)} \nmeta: ${(0, common_1.buf2hex)(this.meta.key)}`,
            verbose: true,
        });
    }
    async connect(opts) {
        ...
        async function connectToNetwork() {
            try {
                const network = (0, network_node_1.default)(network_config);
                network.on('connection', async (socket, peer) => {
                    (0, common_1.emit)({
                        event: 'network.new-connection',
                        ch: 'network',
                        msg: `nid: ${(0, common_1.buf2hex)(self.config.network_id?.publicKey).slice(0, 8)} | address: ${(0, common_1.buf2hex)(self.address_hash).slice(0, 8)}, peers: ${network.peers.size}, conns: ${network.ws.connections.size} - new connection from ${(0, common_1.buf2hex)(peer.peer.host).slice(0, 8)}`,
                    });
        ...
        if (!opts?.local_only) {
            ...
            this.connection_id = (0, common_1.buf2hex)(this.network.webrtc.id);
            (0, common_1.emit)({
                ch: 'network',
    ...
    async getKeys() {
        return {
            data: {
                writers: [...this.dataviewer.inputs.map((i) => (0, common_1.buf2hex)(i.key))],
                indexes: [
                    ...this.dataviewer.outputs.map((i) => (0, common_1.buf2hex)(i.key)),
                    (0, common_1.buf2hex)(this.dataviewer.localOutput.key),
                ],
            },
            meta: {
                writers: [...this.metaviewer.inputs.map((i) => (0, common_1.buf2hex)(i.key))],
                indexes: [
                    ...this.metaviewer.outputs.map((i) => (0, common_1.buf2hex)(i.key)),
                    (0, common_1.buf2hex)(this.metaviewer.localOutput.key),
                ],
            },
        };
    }
    async _updatePeerCache() {
        ...
        if (peers_val?.value) {
            this.peers_cache.peers = (0, common_1.decodeCoreData)(peers_val.value);
        }
        else
            this.peers_cache.peers = {};
        if (trusted_peers_val?.value) {
            this.peers_cache.trusted_peers = (0, common_1.decodeCoreData)(trusted_peers_val.value);
        }
        else
                    ...
                    switch (peers[key].partition) {
                        case 'data':
                            ...
                            if (dataviewer_i >= 0) {
                                ...
                                return (0, common_1.emit)({ ch: 'error', msg: `couldn't snapshot dataviewer core ${key}` });
                            }
                            break;
                        case 'meta':
                            ...
                            if (metaviewer_i >= 0) {
                                ...
                                return (0, common_1.emit)({ ch: 'error', msg: `couldn't snapshot metaviewer core ${key}` });
                            }
                            break;
                        default:
                            (0, common_1.error)('unknown partition');
                            break;
                    }
                }
            }
            ...
            if (key !== this.writer_key) {
                ...
                if (trusted_peers[key].status === 'frozen') {
                    ...
                    if (dataviewer_i >= 0) {
                        ...
                        return (0, common_1.emit)({ ch: 'error', msg: `couldn't dataviewer snapshot core ${key}` });
                    }
                    if (metaviewer_i >= 0) {
                        ...
                        return (0, common_1.emit)({ ch: 'error', msg: `couldn't metaviewer snapshot core ${key}` });
                    }
                }
            }
        }
    ...
    async addPeer(opts) {
        ...
        if (key === (await this.writer_key) || key === (await this.meta_key))
            return null;
        const dataviewer_keys = this.dataviewer.inputs.map((core) => (0, common_1.buf2hex)(core.key));
        const metaviewer_keys = this.metaviewer.inputs.map((core) => (0, common_1.buf2hex)(core.key));
        ...
        switch (partition) {
            ...
            case 'meta':
                ...
                break;
            default:
                return (0, common_1.error)('partition not specified');
        }
        await this._updatePartitions();
        (0, common_1.emit)({
            ch: 'network',
    ...
    async removePeer(opts) {
        ...
        if (destroy) {
            ...
            if (core_i >= 0) {
                ...
                return (0, common_1.emit)({ ch: 'error', msg: `couldn't snapshot core ${key}` });
            }
        }
    ...
    async addTrustedPeer(opts) {
        ...
        switch (partition) {
            ...
            default:
                return (0, common_1.error)('unknown protocol');
        }
        (0, common_1.emit)({
            ch: 'network',
            ...
            switch (partition) {
                ...
                case 'meta':
                    ...
                    if (metaviewer_i >= 0) {
                        ...
                        return (0, common_1.emit)({ ch: 'error', msg: `couldn't snapshot core ${partition}/${key}` });
                    }
                    break;
                default:
                    return (0, common_1.error)('unknown protocol');
...
async function Core(params) {
    ...
    await C.init();
    const API = {
        on: async (id, cb) => () => (0, common_1.subscribeToEvent)({ id, cb }),
        listenLog: async (ch, cb) => () => (0, common_1.subscribeToChannel)({ ch, cb }),
    ...
    const protocolBridge = async function (viewer) {
        return async function (op) {
            ...
            const op_buf = (0, common_1.encodeCoreData)(op);
        ...
        return API({
            ...
            query: async function (params) {
                ...
                return new Promise((resolve, reject) => {
                    const bundle = [];
                    stream.on('data', (data) => {
                        const val = (0, common_1.decodeCoreData)(data.value);
    ...
    API['_allMeta'] = async () => {
        ...
        if (!data)
            return null;
        return (0, common_1.decodeCoreData)(data);
    };
    ...
    return new Promise(async (resolve, reject) => {
        try {
            async function startCore(Protocol, appAPI, UI) {
                ...
                if (!(await API.getNetwork()))
                    await C.connect(params?.config?.connect?.local_only
                        ...
                        : {});
                (0, common_1.emit)({ ch: 'core', msg: `Container initialized successfully` });
            ...
            if (params.app?.Protocol && params.app?.API) {
                (0, common_1.emit)({ ch: 'core', msg: `App provided as argument, loading...` });
                ...
                (0, common_1.emit)({ ch: 'core', msg: `Loading app...` });
                ...
                if (code?.app) {
                    ...
                    (0, common_1.emit)({ ch: 'core', msg: `No code found, querying peers for code, standby...` });
                    ...
                    const interval = setInterval(async () => {
                        ...
                        if (n._peers.size > 0) {
                            (0, common_1.emit)({ ch: 'network', msg: `Got peers, loading code...` });
                            const code = await API['_getMeta']('code');
                            if (code?.app) {
                                ...
                                (0, common_1.emit)({ ch: 'core', msg: `No code found, trying again...` });
                            }
                        }
                        timeout--;
