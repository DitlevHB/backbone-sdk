...
if (typeof window === 'object') {
    appsCache = (0, idb_keyval_1.createStore)('apps-cache', 'backbone');
    ...
    appsCache = {
        get: async (key, store) => {
            ...
            return (0, msgpackr_1.unpack)(raw_data);
        },
        set: async (key, value, store) => {
            fs_1.default.mkdirSync(`${__dirname}/.cache/`);
            fs_1.default.writeFileSync(`${__dirname}/.cache/${key}`, (0, msgpackr_1.pack)(value));
            return true;
        },
    };
}
...
class CoreClass {
    constructor(config, protocol) {
        ...
        const { storage, storage_id } = (0, storage_1.default)(config);
    ...
    async init() {
        ...
        root.on('peer-add', (user) => {
            addPeersExtension.send({
                data: {
                    users: this.dataviewer.inputs.map((core) => (0, common_1.buf2hex)(core.key)),
                    trusted_users: this.dataviewer.outputs.map((core) => (0, common_1.buf2hex)(core.key)),
                },
                meta: {
                    users: this.metaviewer.inputs.map((core) => (0, common_1.buf2hex)(core.key)),
                    trusted_users: this.metaviewer.outputs.map((core) => (0, common_1.buf2hex)(core.key)),
                },
            }, user);
            (0, common_1.emit)({
                ch: 'network',
        ...
        const API = {
            put: async (params) => {
                ...
                if (unsigned) {
                    ...
                    return (0, common_1.error)('unsigned data detected in protocol, discarding...');
                }
                const encoded_data = (0, common_1.encodeCoreData)(params.value);
                if (!encoded_data)
                    return (0, common_1.error)('put needs data');
            ...
            discard: async (op, reason) => {
                ...
                return (0, common_1.error)(`protocol discarded an operation ${reason ? `(${reason})` : ''}: ${JSON.stringify(op).slice(0, 200)}`);
            },
    ...
    async _updateUserCache() {
        ...
        if (users_val?.value) {
            this.users_cache.users = (0, common_1.decodeCoreData)(users_val.value);
        }
        else
            this.users_cache.users = {};
        if (trusted_users_val?.value) {
            this.users_cache.trusted_users = (0, common_1.decodeCoreData)(trusted_users_val.value);
        }
        else
...
function enableGracefulExit(C, config) {
    if (typeof window === 'object') {
        ...
        process.once('SIGINT', async () => {
            (0, common_1.emit)({ ch: 'network', msg: 'SIGINT event received, cleaning up...' });
            await C.network.destroy();
        });
    }
}
async function Core(params) {
    ...
    const API = {
        ...
        meta: {
            getAppVersion: async () => {
                ...
                if (!manifest)
                    return (0, common_1.error)('no manifest found');
    ...
    const protocolBridge = async function (viewer) {
        return async function (op) {
            ...
            if (unsigned) {
                return (0, common_1.error)('unsigned data detected in API');
            }
        ...
        try {
            ...
            if (params.app?.Protocol && params.app?.API) {
                ...
                if (appsCache && !bypassCache) {
                    cached_code = await (0, idb_keyval_1.get)(`${config.address}/code`, appsCache);
                    if (cached_code)
                        cached_code = await (0, msgpackr_1.unpack)(cached_code);
                    let cached_manifest = await (0, idb_keyval_1.get)(`${config.address}/manifest`, appsCache);
                    if (cached_manifest)
                        cached_manifest = await (0, msgpackr_1.unpack)(cached_manifest);
                }
                if (cached_code?.app) {
                    (0, common_1.emit)({ ch: 'core', msg: `App found from cache` });
                    if (logUI)
                    ...
                    const interval = setInterval(async () => {
                        ...
                        if (n._peers.size > 0) {
                            if (!loading_code) {
                                ...
                                (0, common_1.emit)({ ch: 'network', msg });
                                if (logUI)
                                    logUI(msg);
                                ...
                                if (code?.app) {
                                    ...
                                    if (code.signature === '!!!DEV!!!') {
                                        (0, common_1.log)('APP STARTED IN DEV MODE\nWARNING: SECURITY DISABLED');
                                    }
                                    else {
                                    }
                                    const manifest = await API.meta['_getMeta']('manifest');
                                    if (!manifest) {
                                        ...
                                        if (typeof window === 'object' && typeof window['reset'] === 'function')
                                            window['reset']();
                                        return (0, common_1.error)(err);
                                    }
                                    if (appsCache) {
                                        await (0, idb_keyval_1.set)(`${config.address}/code`, (0, msgpackr_1.pack)(code), appsCache);
                                        await (0, idb_keyval_1.set)(`${config.address}/manifest`, (0, msgpackr_1.pack)(manifest), appsCache);
                                    }
                                    if (typeof window === 'object') {
                                        (0, common_1.emit)({ ch: 'core', msg: `Executing in browser environment...` });
                                        ...
                                        (0, common_1.emit)({ ch: 'core', msg: `Executing in NodeJS environment...` });
